...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    ...
    else makeCodeTree(trees.head, trees.tail.head)::combine(trees.tail.tail)
  }
  ...
  def decodeAcc(tree: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = bits match {
    ...
    case head::tail 	=> tree match {
      case Fork(left, right, chars, w) 	=> 
        if (head == 0) decodeAcc(left, tail, acc) else decodeAcc(right, tail, acc)
      case Leaf(char, w) 					=> 
        decodeAcc(tree, tail, char::acc)
    }
  }
